...
saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    //
    double cudaStartTime = CycleTimer::currentSeconds();
    ...
    double cudaEndTime = CycleTimer::currentSeconds();

    double overallCudaDuration = cudaEndTime - cudaStartTime;
    printf("Overall Cuda time: %.3f ms\t\t[%.3f GB/s]\n", 1000.f * overallCudaDuration, toBW(totalBytes, overallCudaDuration));
